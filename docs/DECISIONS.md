...
## ADR-014: Wizard Pha 6 — `MainWindowViewModel` tách theo `partial class` mỗi bước (không tách nhiều ViewModel), thêm `ICsvSchemaConverter.WriteRowsAsync`

**Ngày:** 2026-07-14
**Bối cảnh:** Pha 6 yêu cầu biến màn hình đơn (chỉ có Setup+Scan và
ConfirmFiles) thành 1 wizard bao trọn pipeline: Setup → ConfirmFiles →
ExtractAndExportCsv → Translate → Review → Repack. `MainWindowViewModel` sẽ
cần rất nhiều state/command mới (extract, export CSV, gọi Gemini, sửa CSV
trong app, ghi lại asset, repack).
**Quyết định 1 — tổ chức ViewModel:** Giữ nguyên 1 class
`MainWindowViewModel`/1 `DataContext` duy nhất (đúng cấu trúc app hiện tại,
KHÔNG tạo `WizardHostViewModel` + 6 ViewModel con riêng), nhưng tách thành
nhiều file `partial class` theo tên bước:
`MainWindowViewModel.cs` (host: DI, `WizardStep` enum, `CurrentStep`,
`Next`/`Back`, state dùng chung như `GameProfile` đã detect, map
`VirtualPath -> (ExtractedFilePath, Kind)`, `_lastExtractDirectory`),
`.Setup.cs`, `.ConfirmFiles.cs`, `.ExtractCsv.cs`, `.Translate.cs`,
`.Review.cs`, `.Repack.cs`.
**Lý do:** Nhiều ViewModel riêng cho từng bước sẽ cần 1 lớp điều phối
truyền state qua lại giữa các bước (GameProfile, danh sách file đã extract,
CSV path...) — thêm 1 tầng abstraction không cần thiết cho 1 app 1 màn hình,
1 người dùng. Partial class cho phép tổ chức file theo bước (dễ review từng
phần) mà không phải trả giá kiến trúc đó.
**Đánh đổi chấp nhận:** File `MainWindowViewModel.*.cs` cộng lại khá dài (7
file); các field state dùng chung (`_lastDetectedProfile`,
`_extractedFileLookup`, `_lastExtractDirectory`, `_lastAesKeyHex`,
`_lastUsedUnpackProvider`) nằm ở file host nhưng được ĐỌC/GHI trực tiếp từ
các file bước khác — chấp nhận được vì cùng 1 class (partial), không phải
vi phạm ranh giới module theo CLAUDE.md §5.6 (rule đó áp dụng cho module
Core, không áp dụng cho 1 ViewModel trong App).

**Quyết định 2 — thêm `ICsvSchemaConverter.WriteRowsAsync`:** Interface cũ
chỉ có `ExportAsync(entries)` (luôn ghi `TranslatedText` rỗng, dùng cho lần
xuất CSV đầu tiên từ asset gốc ở bước ExtractAndExportCsv) và
`ImportAsync(path) -> rows` — KHÔNG có cách ghi 1 `IReadOnlyList<CsvRow>` đã
có bản dịch/status ngược lại ra đĩa. Cả bước Translate (ghi lại CSV sau khi
gọi Gemini) và bước Review (nút "Lưu CSV" sau khi người dùng sửa tay trong
app) đều cần thao tác này. Thêm
`Task<Result> WriteRowsAsync(IReadOnlyList<CsvRow> rows, string csvPath, CancellationToken ct)`
vào `ICsvSchemaConverter`, implement trong `CsvSchemaConverter` tái dùng
`CsvRecordMap`/`CsvConfig` sẵn có.
**Lý do:** Đây là 1 lỗ hổng thiết kế còn sót lại từ Pha 4 (lúc đó chưa có UI
